...
def main(n_samples, plot, Tt=80):
    ...
    for i in range(n_train):
        ...
        if len(t_vec)>0:
            gt_t[t_vec[0]+delay_steps] = 1
            
        y_vec = np.zeros((Tt))
        if len(t_vec)>0:
            y_vec[t_vec[0]+delay_steps:] = 1
...
if __name__=='__main__':
    ...
    if not os.path.exists('./data/simulated_delayed_spike_data'):
        os.mkdir('./data/simulated_delayed_spike_data')
    save_data('./data/simulated_delayed_spike_data/x_train.pkl', x_train_n)
    save_data('./data/simulated_delayed_spike_data/y_train.pkl', y_train)
    save_data('./data/simulated_delayed_spike_data/x_test.pkl', x_test_n)
    save_data('./data/simulated_delayed_spike_data/y_test.pkl', y_test)
    save_data('./data/simulated_delayed_spike_data/thresholds_train.pkl', thresholds_train)
    save_data('./data/simulated_delayed_spike_data/thresholds_test.pkl', thresholds_test)
    save_data('./data/simulated_delayed_spike_data/gt_train.pkl', gt_importance_train)
    save_data('./data/simulated_delayed_spike_data/gt_test.pkl', gt_importance_test)
